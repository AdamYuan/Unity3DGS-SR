...
// from "SA-GS"
float3 CalcCovariance2DSAGS(float3 worldPos, float3 cov3d0, float3 cov3d1, float4x4 matrixV, float4x4 matrixP, float4 screenParams)
{
    float4x4 viewMatrix = matrixV;
    float3 viewPos = mul(viewMatrix, float4(worldPos, 1)).xyz;
...
float Calc2DRadius(float3 cov2d) {
    ...
    float lambda1 = mid + radius;
    float lambda2 = max(mid - radius, 0.1);
    return 3.0 * sqrt(max(lambda1, lambda2));
}

float2 Calc2DRect(float3 cov2d) {
    float diag1 = cov2d.x, diag2 = cov2d.z, offDiag = cov2d.y;
    float mid = 0.5f * (diag1 + diag2);
    float radius = length(float2((diag1 - diag2) / 2.0, offDiag));
    float lambda1 = mid + radius;
    float lambda2 = max(mid - radius, 0.1);
    float2 diagVec = normalize(float2(offDiag, lambda1 - diag1));
    float2 v1 = sqrt(lambda1) * diagVec;
    float2 v2 = sqrt(lambda2) * float2(diagVec.y, -diagVec.x);
    return 3.0 * (abs(v1) + abs(v2));
}
